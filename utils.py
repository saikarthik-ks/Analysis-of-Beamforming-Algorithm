...
def Rate_func(temp):
    ...
    # Element-wise inner product per sample
    hv = tf.reduce_sum(h * v, axis=-1, keepdims=True)

    # Compute rate (Shannon capacity formula)
    rate = tf.math.log(tf.cast(1 + (SNR_input / Nt) * tf.pow(tf.abs(hv), 2), tf.float32)) / tf.math.log(2.0)
    return -rate  # negative for loss minimization
